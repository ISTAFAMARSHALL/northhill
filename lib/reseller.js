...
export async function createResellerLine({ planName, planTerm, connections, startDate, description }) {
  ...
  try {
    // Navigate to the create form — this ensures JS-populated hidden fields
    // (e.g. server_id, bouquet checkboxes) are already present in the DOM
    try {
    ...
    console.log("[reseller] Create page URL:", page.url(), "| title:", await page.title());

    // Fill every known field using the actual DOM — sets value on hidden inputs
    // too (e.g. Select2-backed current_bouquets), dispatches change events
    ...
    // Snapshot the full FormData so we can see every field (including hidden ones)
    const formSnapshot = await page.evaluate(() => {
      const form = document.querySelector("form");
      if (!form) return "NO FORM FOUND";
      const entries = [];
      for (const [k, v] of new FormData(form).entries()) entries.push(`${k}=${v}`);
      return entries.join("&");
    });
    console.log("[reseller] Form snapshot before submit:", formSnapshot);

    // Click submit and wait for the resulting navigation
    await Promise.all([
    ...
    if (!finalUrl.includes("/users") && !finalUrl.includes("/lines")) {
      // Try to surface any flash error the panel rendered
      const flashText = await page.evaluate(() => {
        const el = document.querySelector(".alert, .flash, [class*='error'], [class*='danger']");
