...
export async function createResellerLine({ planName, planTerm, connections, startDate, description }) {
  ...
  try {
    // Navigate to the create page first — this gives us a fresh CSRF token
    // that is tied to the current page load (more reliable than the post-login one)
    try {
    ...
    console.log("[reseller] CSRF from create page:", !!freshCsrf);

    const result = await page.evaluate(async (panelUrl, csrf, bouquets, pkg, isTrial, username, password, desc) => {
      // Field order must match the real browser form submission exactly:
    ...
    // Success = the panel accepted the POST and redirected (to /dashboard or /users —
    // this panel redirects to /dashboard on success, confirmed by local testing).
    // Failure = 422 validation error, or a non-redirect 200 (form re-rendered with errors).
    ...
    if (!result.redirected) {
      throw new Error(`Line creation failed — no redirect (form likely re-rendered with errors). Status: ${result.status}, URL: ${result.url}`);
    }
