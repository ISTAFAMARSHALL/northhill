...
export async function createResellerLine({ planName, planTerm, connections, startDate, description }) {
  ...
  //   non-trial → page: /lines/create/0/line  POST: /lines/create/0
  //   trial     → page: /lines/create/1/line  POST: /lines/create/1
  // Response is JSON (Content-Type: application/json), not an HTML redirect.
  const createPagePath = isTrial ? "/lines/create/1/line" : "/lines/create/0/line";
  const createPostPath = isTrial ? "/lines/create/1"      : "/lines/create/0";

  try {
    // Navigate to the correct create page to get a fresh CSRF token
    try {
      await page.goto(`${PANEL_URL}${createPagePath}`, { waitUntil: "domcontentloaded", timeout: 30000 });
    } catch (e) {
    ...
    console.log("[reseller] CSRF from create page:", !!freshCsrf, "| endpoint:", createPostPath);

    const result = await page.evaluate(async (panelUrl, postPath, pagePath, csrf, bouquets, pkg, isTrial, username, password, desc) => {
      // Field order confirmed from DevTools capture of a real browser submission
      ...
      const res = await fetch(`${panelUrl}${postPath}`, {
        method:      "POST",
        credentials: "include",
        headers: {
          "Content-Type":     "application/x-www-form-urlencoded; charset=UTF-8",
          "X-Requested-With": "XMLHttpRequest",                        // required — panel checks this
          "X-CSRF-TOKEN":     csrf,
          "Accept":           "application/json, text/javascript, */*; q=0.01",
          "Referer":          `${panelUrl}${pagePath}`,
          "Origin":           panelUrl,
        },
        body: body.toString(),
        // No redirect:"follow" — endpoint returns JSON directly (confirmed 200 + application/json)
      });
    ...
    }, PANEL_URL, createPostPath, createPagePath, tokenToUse, BOUQUETS, packageId, isTrial, username, password, description || planName);

    console.log("[reseller] Sent body:", result.sentBody);
    console.log("[reseller] Create status:", result.status, "| url:", result.url);
    console.log("[reseller] Response JSON:", JSON.stringify(result.json));

    if (result.status !== 200) {
      throw new Error(`Unexpected status ${result.status}: ${result.raw.slice(0, 200)}`);
    }

    // Parse the JSON response — log it fully so we can learn what success looks like
    if (result.json?.error || result.json?.errors || result.json?.status === "error") {
      throw new Error(`Panel rejected line: ${JSON.stringify(result.json)}`);
    }
