...
function LoginScreen({ onLogin }) {
  ...
  const handleOAuth = async (provider) => {
    ...
    setError("");
    const redirectTo = `${window.location.origin}/auth/done?next=/portal`;
    const { error } = await supabase.auth.signInWithOAuth({ provider, options: { redirectTo } });
    if (error) {
      setError(error.message);
      setOauthLoading("");
    }
    // On success Supabase redirects the page — no further handling needed
  };
  ...
  const oauthBtnStyle = (name) => ({
    ...
    color: "#e8e8f0", fontSize: 14, fontWeight: 500, cursor: "pointer",
    transition: "background 0.15s ease",
    opacity: oauthLoading && oauthLoading !== name ? 0.5 : 1,
  });
    ...
    <div style={{ ...S.page, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .oauth-btn:hover { background: rgba(255,255,255,0.1) !important; }
      `}</style>
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ ...S.card, padding: "2.25rem" }}>
          ...
          <p style={{ fontSize: 14, color: "#6b7280", marginBottom: "1.5rem" }}>Sign in to view your subscription</p>

          {/* ── Social login buttons ── */}
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", marginBottom: "1.25rem" }}>
            <button className="oauth-btn" style={oauthBtnStyle("google")} onClick={() => handleOAuth("google")} disabled={!!oauthLoading}>
              {/* Google icon */}
            ...
            <button className="oauth-btn" style={oauthBtnStyle("apple")} onClick={() => handleOAuth("apple")} disabled={!!oauthLoading}>
              {/* Apple icon */}
            ...
            <button className="oauth-btn" style={oauthBtnStyle("facebook")} onClick={() => handleOAuth("facebook")} disabled={!!oauthLoading}>
              {/* Facebook icon */}
          ...
          <form onSubmit={handleLogin}>
            ...
            <button type="submit" className="login-submit" style={{ ...S.btnPrimary, transition: "opacity 0.15s ease" }} disabled={loading || !!oauthLoading}>
